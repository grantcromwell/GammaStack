...
class SetGraph:
    ...
    def set_graph(self):
        ...
        agent_nodes = {}
        tool_nodes = {}
        # Updated agent list with new advanced features
        all_agents = ["indicator", "pattern", "trend", "correlation", "market_neutral"]

        # create nodes for indicator agent (ENHANCED with plausibility)
        ...
        # create nodes for trend agent (ENHANCED with fair value analysis)
        agent_nodes["trend"] = create_trend_agent(
            self.agent_llm, self.graph_llm, self.toolkit
        )
        # tool_nodes["trend"] = self.tool_nodes["trend"]

        # create nodes for NEW correlation topology agent
        agent_nodes["correlation"] = create_correlation_topology_agent(self.agent_llm)

        # create nodes for NEW market neutral risk manager agent
        agent_nodes["market_neutral"] = create_market_neutral_agent(self.agent_llm)
        ...
        # add edges to graph in enhanced order
        # Indicator → Pattern → Trend → Correlation → Market Neutral → Decision
        for i, agent_type in enumerate(all_agents):
            current_agent = f"{agent_type.capitalize()} Agent"

            if i == len(all_agents) - 1:
                # Last agent connects to Decision Maker
