...
Enhanced with Fair Value (Residual Analysis) for Mean Reversion Detection.
"""
...
def create_trend_agent(tool_llm, graph_llm, toolkit):
    ...
    Enhanced with fair value and mean reversion detection.
    """
    ...
    def trend_agent_node(state):
        ...
        # --- NEW: Compute fair value and residual analysis ---
        fair_value_model = {
            'fair_price': None,
            'current_price': None,
            'deviation_pct': 0,
            'r_squared': 0,
            'recommendation': 'Fair valued'
        }
        
        try:
            if state.get("kline_data") and "Close" in state["kline_data"]:
                df = pd.DataFrame(state["kline_data"])
                if len(df) > 20:
                    # Build simple predictors from available data
                    # Using normalized prices and simple metrics
                    predictors = {
                        'price_normalized': ((df['Close'] - df['Close'].min()) / (df['Close'].max() - df['Close'].min() + 1e-8)).tolist(),
                        'volume_normalized': ((df['Volume'] - df['Volume'].min()) / (df['Volume'].max() - df['Volume'].min() + 1e-8)).tolist() if 'Volume' in df else [0.5] * len(df),
                    }
                    
                    analyzer = ResidualAnalyzer()
                    model = analyzer.build_fair_value_model(state["kline_data"], predictors, lookback=50)
                    
                    fair_value_model = {
                        'fair_price': float(model['fair_prices'][-1]) if model['fair_prices'] else None,
                        'current_price': float(df['Close'].iloc[-1]),
                        'deviation_pct': float(model['current_discount_pct']),
                        'r_squared': float(model['r_squared']),
                        'recommendation': 'Mean revert to fair' if abs(model['current_discount_pct']) > 5 else 'Fair valued'
                    }
        except Exception as e:
        ...
        if not trend_image_b64:
            ...
            system_prompt = (
                ...
                f"NEW FEATURE - FAIR VALUE ANALYSIS:\n"
                f"Fair Price (regression model): ${fair_value_model['fair_price']:.2f}\n"
                f"Current Price: ${fair_value_model['current_price']:.2f}\n"
                f"Deviation: {fair_value_model['deviation_pct']:.2f}%\n"
                f"Model Quality (R²): {fair_value_model['r_squared']:.2f}\n"
                f"Recommendation: {fair_value_model['recommendation']}\n\n"
                f"Use this fair value context to bias your trend analysis towards mean reversion setups when price is significantly mispriced (>5% deviation)."
            )
